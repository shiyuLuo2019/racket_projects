...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname assignment6) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
;;;;; Assignment 6
(require 2htdp/image)
...
(check-expect (convert-euro '(1 2)) '(1.22 2.44))
(define (convert-euro l)
  (map (lambda (usd) (* usd 1.22)) l))
...
(check-expect (convertFC '(34)) (list (/ 10 9)))
(define (convertFC l)
  (map (lambda (f) (* (- f 32) (/ 5 9))) l))
...
(define-struct ir (name description acq sales))
;; An IR(Inventory Record) is a (make-ir String String Number Number)

;; eliminate-expensive: Number [List-of IR] -> [List-of IR]
;; produces a list of all those structures whose sales price is below ua
(check-expect (eliminate-expensive '() 5.00) '())
(check-expect (eliminate-expensive
               (list (make-ir "Chips" "Sea salt flavor" 2.19 3.99)
                     (make-ir "Sausages" "Italian Sweet" 2.39 4.99)
                     (make-ir "Beef Steak" "Slightly Seasoned" 5.99 6.99))
               5.00)
              (list (make-ir "Chips" "Sea salt flavor" 2.19 3.99)
                    (make-ir "Sausages" "Italian Sweet" 2.39 4.99)))
(check-expect (eliminate-expensive
...
(define (eliminate-expensive lir ua)
  (local [; add-cheap: IR [List-of IR] -> [List-of IR]
          ; cons i onto l if sales price of i is below ua
          (define (add-cheap i l)
            (if (< (ir-sales i) ua)
                (cons i l) l))]
    (foldr add-cheap '() lir)))
...
;; create-even: PosInt -> [List-of Number]
;; creates a list composed of the first n even numbers
(check-expect (create-even 1) '(1))
(check-expect (create-even 2) '(1 3))
(check-expect (create-even 3) '(1 3 5))
(define (create-even n)
  (build-list n (lambda (x) (+ (* x 2) 1))))


;;; Problem 4

;; append-from-foldr: [List-of ITEM] [List-of ITEM] -> [List-of ITEM]
;; appends the second list to the end of the first list
(check-expect (append-from-foldr '() '()) '())
(check-expect (append-from-foldr '(1 2) '()) '(1 2))
(check-expect (append-from-foldr '() '(1 2)) '(1 2))
(check-expect (append-from-foldr '(1 2) '(3 4)) '(1 2 3 4))
(define (append-from-foldr l1 l2)
  (foldr (lambda (item l) (cons item l)) l2 l1))

;; if foldr is replaced by foldl
;; the order of the second list in the result would be reversed.
;; example:
;; (append-from-foldl '(1 2) '(3 4)) -> '(2 1 3 4)

;;; Problem 5

;; map*: [X -> Y] [List-of X] -> [List-of Y]
;; behaves exactly like map
(check-expect (map* add1 '()) '())
(check-expect (map* add1 '(1 2 3)) '(2 3 4))
(check-expect (map* string-length '("bunny" "tigi")) '(5 4))
(define (map* f l)
  (local [; g: X Y -> Y
          ; functions as f on item and cons the result to base
          (define (g item base)
            (cons (f item) base))]
    ; [X Y -> Y] Y [List-of X] -> [List-of Y]
    (foldr g '() l)))
